-- Update survey_cumulative_stats view and get_survey_cumulative_summary function
-- to support filtering by instructor_id
...
-- 2. Recreate View with instructor_ids
CREATE VIEW public.survey_cumulative_stats AS
WITH instructor_union AS (
  SELECT s.id AS survey_id, i.id AS instructor_id, i.name AS instructor_name
  ...
  SELECT ss.survey_id, i2.id AS instructor_id, i2.name AS instructor_name
...
instructor_agg AS (
  SELECT
    ...
    ARRAY_AGG(DISTINCT instructor_id) AS instructor_ids,
...
SELECT
  ...
  COALESCE(ia.instructor_ids, ARRAY[]::uuid[]) AS instructor_ids,
...
-- 3. Recreate Function with instructor_id parameter
  ...
  WITH filtered AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR instructor_id = ANY(scs.instructor_ids)
      )
      AND (
...
COMMENT ON FUNCTION public.get_survey_cumulative_summary(text, integer, text, boolean, uuid) IS
  'Returns summary metrics for the survey_cumulative_stats view after applying dashboard filters, including instructor_id.';
