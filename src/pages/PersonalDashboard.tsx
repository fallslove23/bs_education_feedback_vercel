...
import {
  ...
  Lightbulb, ThumbsUp, ThumbsDown, FileText
} from 'lucide-react';
...
export default function PersonalDashboard({ targetInstructorId }: PersonalDashboardProps) {
  ...
  const [sourceSurveys, setSourceSurveys] = useState<SourceSurvey[]>([]);
  ...
  const handlePdfDownload = async () => {
    const success = await downloadElementAsPdf('personal-dashboard-content', {
      filename: `개인통계리포트_${new Date().toISOString().slice(0, 10)}.pdf`,
      scale: 2,
      excludeClassNames: ['no-print']
    });

    if (success) {
      toast({ title: 'PDF 다운로드 완료', description: '리포트가 성공적으로 저장되었습니다.' });
    } else {
      toast({ title: 'PDF 저장 실패', description: 'PDF 생성 중 오류가 발생했습니다.', variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div id="personal-dashboard-content" className="space-y-4 sm:space-y-6 p-1">
        {/* Filters */}
        <Card>
          <CardHeader>
      ...
      <Dialog open={showSourceSurveys} onOpenChange={setShowSourceSurveys}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>집계된 설문 목록</DialogTitle>
            <CardDescription>
              현재 통계에 포함된 {sourceSurveys.length}개의 설문 데이터입니다.
            </CardDescription>
          </DialogHeader>
                ...
                <TableBody>
                  {sourceSurveys.map((survey) => (
                    <TableRow key={survey.id}>
                      <TableCell className="font-medium">{survey.course_name}</TableCell>
                      <TableCell>{survey.course_round || '-'}</TableCell>
                      <TableCell>{survey.instructor_name}</TableCell>
                      <TableCell>
                        <Badge variant={survey.average_score >= 9 ? 'default' : 'secondary'}>
                          {survey.average_score?.toFixed(1) || '-'}
                        </Badge>
                      </TableCell>
                      <TableCell>{survey.response_count}명</TableCell>
                      <TableCell>{new Date(survey.end_date).toLocaleDateString()}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
